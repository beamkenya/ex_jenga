defmodule ExJenga.JengaBase do
  @moduledoc false

  import ExJenga.Util
  alias ExJenga.TokenServer
  ...
  def auth_client() do
    middleware = [
      ...
      {Tesla.Middleware.Headers,
       [
         {"Authorization", "Basic " <> Application.get_env(:ex_jenga, :jenga)[:api_key]},
  ...
  defp generate_token(client) do
    case Tesla.post(client, "/identity/v2/token", %{
           "username" => Application.get_env(:ex_jenga, :jenga)[:username],
           "password" => Application.get_env(:ex_jenga, :jenga)[:password]
         })
         |> get_token do
      {:ok, token} ->
